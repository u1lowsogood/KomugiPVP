...
import cx.myhome.u1lowisland.komugipvp.game.GameModeEnum;
...
public class SelectMapPlayMenu extends Menu {

    public SelectMapPlayMenu(PlayerMenuUtility playerMenuUtility) {
    ...
    public void handleMenu(InventoryClickEvent e) throws MenuManagerNotSetupException, MenuManagerException {
        ...
        if (currentItem.isSimilar(MenuItemEnum.CLOSE.getItemStack())) {
            ...
            String mapName = currentItem.getItemMeta().getDisplayName();
            GameMap gameMap = (GameMap) ServiceLocator.getInstance().getMapConfig().getConfig().get(mapName);
            GameManager gm = ServiceLocator.getInstance().getGameManager();
            gm.gameStart(gameMap, GameModeEnum.FFA);
        }
